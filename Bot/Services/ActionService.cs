...
namespace Bot.Services
{
    public class ActionService
    {
        ...
        public List<BotAction> GetAll()
        {
            ...
            foreach (Type type in Assembly.GetAssembly(typeof(Action)).GetTypes().Where(myType => myType.IsClass && !myType.IsAbstract && myType.IsSubclassOf(typeof(Action))))
        ...
        private void PurgeAllGuildCommands()
        {
            _ = Task.Run(async () =>
            {
                ...
                {
                    await (await _discord.Rest.GetGuildApplicationCommands(guildSummary.Id)).FirstOrDefault()?.DeleteAsync();
