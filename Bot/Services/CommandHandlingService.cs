...
namespace Bot.Services
{
    public class CommandHandlingService
    {
        ...
        {
            return message.HasStringPrefix("w!", ref argPos) || message.HasMentionPrefix(client.CurrentUser, ref argPos);
        }
        ...
        public async Task CommandExecutedAsync(Optional<CommandInfo> command, ICommandContext context, IResult result)
        {
            if (result?.Error != null && result.Error.Value == CommandError.Exception && result is ExecuteResult executeResult)
            {
                if (executeResult.Exception is CommandInvalidException)
                    await _commands.ExecuteAsync(context, "speak", _services);
                else
                {
